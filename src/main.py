...
#     print(
#         f"""
#         ping echo reply from {ip_header.src}: icmp_seq={echo_reply.seq} ttl={ip_header.ttl}""""
#     )


def ping(host: str, seq: int) -> Tuple[IPHeader, ICMPEcho]:
    with raw_socket() as sock:
        packet = ICMPEcho(ICMPType.ECHO, 0, 100, seq, b"\xff").to_bytes()
